...
DROP TABLE COMMON, COMMON_CODE;
DROP TABLE TERMS, MEMBER_TERMS_AGREEMENT;
DROP TABLE NOTICE, QNA, FAQ;
DROP TABLE RESERVATION, SEAT;
DROP TABLE THEATER, RUN_SCHEDULE, WATCH_HISTORY;
DROP TABLE CREDIT_CARD, ORDER_HISTORY;
DROP TABLE MOVIE, GENRE, CREATOR, VOD;
DROP TABLE MEMBERS;
SET FOREIGN_KEY_CHECKS = 1;

CREATE TABLE COMMON(
    COMMON_ID VARCHAR(20) AUTO_INCREMENT PRIMARY KEY,
    COMMON_CODE VARCHAR(50) UNIQUE NOT NULL,
    COMMON_CODE_CREATE_DATE DATETIME DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE COMMON_CODE(
    COMMON_CODE_ID VARCHAR(20) AUTO_INCREMENT PRIMARY KEY,
    ...
    KEY IDX_COMMON_VALUE (COMMON_CODE) -- 단독 인덱스 추가
);
...
CREATE TABLE MEMBERS(
    ...
    FOREIGN KEY (LANG) REFERENCES COMMON_CODE (COMMON_VALUE) ON DELETE SET NULL
    FOREIGN KEY (USER_TYPE) REFERENCES COMMON_CODE (COMMON_VALUE) ON DELETE SET NULL
);
